...
param (
    ...
    [string]$region,
    [string]$password,
    [string]$salesworkspaceid,
    [string]$financeworkspaceid
)
# else {
    ...
    Start-Sleep -s 1
    $loginCommand = "az login --service-principal --username $clientId --password=`"$clientSecret`" --tenant $tenantId"
    ...
    $response = az ad sp show --id $clientId | ConvertFrom-Json
    ...
    $body = $body.Replace("#userPrincipalName#", $response.appId)
    $body = $body.Replace("#displayName#", $response.displayName)
    $body = $body.Replace("#companyName#", "N/A")
    $body = $body.Replace("#mail#", "N/A")
    ...
    $wsIdContosoSales =  $salesworkspaceid
    $wsIdContosoFinance =  $financeworkspaceid
